...
**시연 도메인**: 「온라인쇼핑몰」 — 9개 이벤트(접수→처리→완료) · 5개 마이크로서비스 후보 · 핵심 외부 시스템 · 정책·핫스팟 자동 도출.
...
### 입력 XLSX — 「온라인쇼핑몰」 표준 입력 형식

📂 [`samples/01-ecommerce-events.xlsx`](samples/01-ecommerce-events.xlsx) — 17 컬럼 표준 입력 (5 시트 · 15 KB)

이 파일을 라이브 데모 사이트에서 **📂 엑셀 불러오기** 로 업로드하면 아래 도메인의 풀 스토밍 결과를 그대로 재현할 수 있습니다:
